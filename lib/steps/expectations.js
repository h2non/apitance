...
step['[should]?[\\s+]?fail due to an error$'] = stepify(function (error) {
  if (!this.error) {
    throw new Error('No error found. The request was success')
  }
  expect(this.error).to.not.empty
})

step['response header "([^"]*)" should (exist|be present)$'] = stepify(function (header) {
  expect(this.response).to.have.header(header)
})

step['response header "([^"]*)" should not (exist|be present)$'] = stepify(function (header) {
  expect(this.response).to.not.have.header(header)
})

step['response header "([^"]*)" should be equal "([^"]*)"$'] = stepify(function (header, value) {
  expect(this.response).to.have.header(header.toLowerCase(), value)
})
...
step['response cookie "([^"]*)" should (exist|be present)$'] = stepify(function (name) {
  expect(req).to.have.cookie(name)
})

step['response cookie "([^"]*)" should not (exist|be present)$'] = stepify(function (name) {
  expect(req).to.not.have.cookie(name)
})

step['response cookie "([^"]*)" equal to "([^"]*)"$'] = stepify(function (name, value) {
  expect(req).to.have.cookie(name, value)
})

step['response body should be equal:$'] = stepify(function (body) {
  expect(this.response.body).to.match(new RegExp(body))
})
