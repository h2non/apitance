...
function runCucumber(flags) {
  ...
  cucumber.run(succeeded => {
    ...
    const timeoutId = setTimeout(() => {
      console.error('Apitance process timed out after waiting 30 seconds ' +
        'for the JavaScript event loop to empty. ' +
        'There may be a resource leak. ' +
        'Have all resources like database ' +
        'connections or network connections been closed properly?')
      process.exit(code)
    }, 30 * 1000)
